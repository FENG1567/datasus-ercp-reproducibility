...
# R-only renderer for the frozen Stage 7 rc_v1_2 source-data release.
...
expected_contract_sha <- "a0648662bdd5bd02dd330acc294f8d0a79037b3ea30e96f583d91f616938a107"
expected_source_manifest_sha <- "096c8cbd7825c6f762490c744036835d704d7e8467b0eb74676bee0dffbdbfb9"
expected_source_audit_sha <- "b5609bcacd04f354a07f2d89e4a0574bc96a586e03630147fbcba3ba32034fc7"
...
text_panel <- function(title, lines, colour = palette[["dark"]]) {
  ...
  ggplot() + annotate("text", x = 0, y = 1, label = title, hjust = 0, vjust = 1, size = 2.5, fontface = "bold", colour = colour) +
    ...
    theme(plot.tag = element_text(size = 8, face = "bold", colour = palette[["dark"]]), plot.tag.position = c(0.01, 0.99))
}
...
p3a <- ggplot(equity, aes(quintile, age_sex_standardised_rate_per_100k, colour = exposure_short)) +
  ...
  scale_colour_manual(values = c(palette[["blue"]], palette[["orange"]])) +
  labs(title = "Directly standardised treated-use rates", subtitle = "Municipality-year ecological strata; bars show frozen 95% intervals", x = "Contextual quintile", y = "AIHs per 100,000 adults", colour = NULL)
...
service_obs <- service[service$observed_treatment %in% TRUE & !is.na(service$main_destination_share), ]
p5b <- ggplot(service_obs, aes(main_destination_share, colour = cohort)) + stat_ecdf(linewidth = 0.55) +
  scale_colour_manual(values = c(A = palette[["blue"]], B = palette[["teal"]])) +
  scale_x_continuous(labels = function(x) paste0(round(100 * x), "%"), limits = c(0, 1)) +
  labs(title = "Service-area modal-destination share", subtitle = "Pooled municipalities with observed treatment; non-spatial summary", x = "Main-destination share", y = "Cumulative share", colour = "Cohort")
...
fig5 <- p5a / (p5b | p5c) + plot_layout(heights = c(1.35, 1)) + plot_annotation(tag_levels = "a", title = "Intermunicipal patient-flow network summaries and referral concentration", subtitle = "Non-spatial observed-flow summaries do not establish poor access or inappropriate referral.")
...
points$metric_label <- factor(metric_labels[points$metric], levels = unname(metric_labels))
...
fig6 <- p6a / (p6b | p6c) + plot_layout(heights = c(1.25, 1)) + plot_annotation(tag_levels = "a", title = "Hospital treatment volume and in-hospital death", subtitle = "Associational point estimates; no causal interpretation.")
...
fig7 <- p7a / (p7b | p7c) + plot_layout(heights = c(1.3, 1)) + plot_annotation(tag_levels = "a", title = "Network resilience under targeted hub removal versus random removal")
...
for (stem in names(table_specs)) {
  ...
  display_data <- data
  if (stem == "Table_4") {
    stage06_status <- display_data$result_id == "stage06.final_status"
    display_data$value[stage06_status] <- gsub("_", " ", as.character(display_data$value[stage06_status]), fixed = TRUE)
  }
  save_table(display_data, stem, spec$title, 183, spec$height)
...
audit_lines <- c(
  "schema_version=stage07_render_audit_rc_v1_2",
  ...
  "Figure_5=No spatial network layout was fabricated because frozen node coordinates are absent; non-spatial observed hub, service-area modal-destination-share, and centrality distributions are shown.",
